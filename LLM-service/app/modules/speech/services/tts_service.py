import os
from app.modules.speech.engines.tts.edge_tts_engine import tts_engine
...
class TTSService:
    ...
    async def generate_speech(text: str, language: str = "en", engine: str = "auto") -> str:
        ...
        if engine == "parler" or (engine == "auto" and language != "en" and language != "hi"):
            try:
                ...
                log_with_prefix("TTSService", f"❌ Parler engine failed: {str(e)}. Falling back to EdgeTTS.", level="warning")
                file_path = None
        
        # 2. Tier 2: EdgeTTS (for English/Hindi or as a robust fallback)
        if not file_path:
            try:
                log_with_prefix("TTSService", "➡️ Step 2: Synthesizing with EdgeTTS...")
                file_path = await tts_engine.synthesize(text, language)
                if file_path:
                    log_with_prefix("TTSService", "✅ EdgeTTS synthesis successful")
            except Exception as e:
