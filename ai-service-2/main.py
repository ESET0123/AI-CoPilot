...
def safe_sql_check(sql: str) -> (bool, str):
    ...
    if ";" in s:
        return False, "Unsafe SQL blocked — no semicolons allowed"
    # must start with SELECT
    if not re.match(r"^\s*SELECT\b", s, flags=re.I):
        return False, "Unsafe SQL blocked — must start with SELECT"
    # forbid dangerous keywords
    for patt in FORBIDDEN_SQL_PATTERNS:
        if re.search(patt, s, flags=re.I):
            return False, f"Unsafe SQL blocked — forbidden pattern detected"
    # query length guard
    if len(s) > 8000:
        return False, "Unsafe SQL blocked — query too long"
    return True, "OK"
...
# ───────────────── RULE-BASED INTENT DETECTION & SQL TEMPLATES ───────────────── #
...
def detect_intent_and_params(question: str) -> dict:
    ...
    # --- Revenue RANGE detection: “from 2025 to 2040”, “2025-2035”, etc ---
    ...
    # Phrases like “next year / coming year”
    if "next year" in q_low:
...
logging.info("✅ Database file found" if os.path.exists(DB_PATH) else "❌ DB NOT FOUND")
...
def startup_event():
    logging.info("⚡ FASTAPI loaded — forecasting + analytics + chatbot ready ⚡")
...
def forecast(meter_id: str = "MTR001", hours: int = 24):
    logging.info("🔥 Forecast endpoint called for meter_id=%s, hours=%s", meter_id, hours)
...
def analytics(payload: dict = Body(...)):
    ...
    if task == "correlation_weather_load":
        ...
        return {"message": f"Temperature → Load correlation = {corr:.3f}", "chart_data": df[["ts", "load_kw"]].to_dict(orient="records")}

    # Correlation: humidity
    if task == "correlation_humidity_load":
        corr = float(df["load_kw"].corr(df["humidity_pct"]))
        return {"message": f"Humidity → Load correlation = {corr:.3f}", "chart_data": df[["ts", "load_kw"]].to_dict(orient="records")}
    ...
    if task == "billing_estimation":
        ...
        return {"message": f"Estimated bill = ₹{est:.2f}", "chart_data": None}
...
# ───────────────── ASK ENDPOINT (HYBRID — RULES → TEMPLATE → RAG+LLM) ───────────────── #
@app.post("/ask")
...
# ------------------- REVENUE FORECAST HANDLING -------------------
    ...
    # ------------------------------------------------------------------
    # 4) FALLBACK = RAG → DeepSeek → SQL or JSON
    # ------------------------------------------------------------------
...
"""
    ...
    # ------------------------------------------------------------------
    # STEP C — CONVERSATIONAL FALLBACK
    # ------------------------------------------------------------------
...
# ───────────────── CHAT ENDPOINT (VISUALIZATION READY) ───────────────── #
